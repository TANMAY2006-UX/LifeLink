...
function wireModalButtons() {
  const openSignInBtn = $('#loginBtn'), openRegisterBtn = $('#registerBtn'), openDonateCta = $('#open-donate-cta'), openReceiveCta = $('#open-receive-cta');
  if (openSignInBtn && signInModal) openSignInBtn.addEventListener('click', (e) => { e.preventDefault(); signInModal.style.display = 'flex'; document.body.style.overflow = 'hidden'; });
  if (openRegisterBtn && registerModal) openRegisterBtn.addEventListener('click', (e) => { e.preventDefault(); registerModal.style.display = 'flex'; document.body.style.overflow = 'hidden'; });
  if (openDonateCta) openDonateCta.addEventListener('click', (e) => { e.preventDefault(); if (isUserSignedIn) { showPage('donation-page'); prefillDonationFormMain(); setInfoFieldsReadonly(true); } else donateOptionsModal && (donateOptionsModal.style.display = 'flex'); });
  if (openReceiveCta && receiveOptionsModal) openReceiveCta.addEventListener('click', (e) => { e.preventDefault(); receiveOptionsModal.style.display = 'flex'; });

  $$('.auth-modal .close-btn').forEach(btn => btn.addEventListener('click', (ev) => { const modal = ev.target.closest('.auth-modal'); if (modal) { modal.style.display = 'none'; document.body.style.overflow = ''; } }));
  $$('.auth-modal').forEach(modal => modal.addEventListener('click', (ev) => { if (ev.target === modal) { modal.style.display = 'none'; document.body.style.overflow = ''; } }));
  document.addEventListener('keydown', (ev) => { if (ev.key === 'Escape') { $$('.auth-modal').forEach(m => { m.style.display = 'none'; }); $$('.dropdown .dropdown-content').forEach(m => m.style.display = 'none'); document.body.style.overflow = ''; } });
}
